...
def test_download(request, subject_id, test_id):
    ...
    for question in download_test.question_set.all():
        content += '// Start of question: ВопрМножВыбор' + '\n'
        content += str(question.question) + ' {' + '\n'

        if question.has_multiple_choice:
            for answer in question.answer_set.all():
                if answer.is_true:
                    content += '~%' + str(answer.weight) + '%' + answer.answer + '\n'
                else:
                    content += '~%' + str(answer.weight) + '%' + answer.answer + '\n'
        else:
            for answer in question.answer_set.all():
    ...
    # download_test = get_object_or_404(Testing, pk=test_id)
    # file_name = translit(download_test.test_name, 'ru', reversed=True) + '.txt'
    #
    # file_path = os.path.join(BASE_DIR, file_name)

    # print(translit(download_test.test_name, 'ru', reversed=True))
